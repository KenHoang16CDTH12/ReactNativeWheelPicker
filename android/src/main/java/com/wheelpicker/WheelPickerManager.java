...
public class WheelPickerManager extends SimpleViewManager<LoopView> implements LoopListener {
    ...
    public void setSelectedItemFont(LoopView wheelPicker, String itemTextFontFamily) {
        if (wheelPicker != null) {
            Typeface typeface = ReactFontManager.getInstance().getTypeface(itemTextFontFamily, Typeface.NORMAL,
    ...
    public void onItemSelect(LoopView picker, int item) {
        if (wheelPicker != null) {
            ...
            ((ReactContext) wheelPicker.getContext()).getJSModule(RCTEventEmitter.class).receiveEvent(picker.getId(),
                    "topChange", event);
        }
    }
